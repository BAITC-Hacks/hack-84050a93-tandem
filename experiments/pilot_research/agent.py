"""Pilot allocation experiments copied from base c65003c5ec8811b829b583bc98e9fe79954b6538.

Sized Agent: first probe <=40 and <=one quarter of a small cell (minimum 10);
repeats use original 100/200 choices. StopAgent: original sizes, but stop after
broad negative evidence. Neither reads effect models or changes shared modules.
...
BASE_SHA = "c65003c5ec8811b829b583bc98e9fe79954b6538"
...
class Agent:
    ...
    def act(self, env):
        self.last_trace = {"pilots": [], "warnings": [], "version": "pilot-research-v1", "policy": self.policy, "base_sha": BASE_SHA}
        ...
        prior = HistoricalPrior(env.tariffs, Path(__file__).resolve().parents[2] / "data" / "change_tariff.csv", reference_multiplier)
        ...
        for step in range(pilot_limit):
            ...
            if self.policy == "stop" and step >= 6:
                tested = [b for b in beliefs if b.pilots]
                estimates = [b.posterior() for b in tested]
                precision = sum(1.0 / std**2 for _, std in estimates)
                pooled_mean = sum(mean / std**2 for mean, std in estimates) / precision
                pooled_upper = pooled_mean + 1.65 / sqrt(precision)
                negative = sum(mean < 0 for mean, _ in estimates)
                strong_positive = any(mean > 1.65 * std for mean, std in estimates)
                covered = {b.segment for b in tested} == set(initial_segments)
                # A heuristic guard against continued loss, not a confidence bound
                # on untested offers. Diverse cells can violate the pooling premise.
                if covered and negative >= 0.8 * len(tested) and not strong_positive and pooled_upper < 0:
                    self.last_trace["stop"] = {"reason": "broad_negative_evidence", "step": step,
                                               "pooled_upper": pooled_upper, "tested": len(tested)}
                    break
            ...
            for belief in pool:
                ...
                sizes = (100, 200)
                if self.policy == "sized" and not belief.pilots:
                    sizes = (min(40, max(10, cell["n"] // 4)),)
...
class StopAgent(Agent):
    """Alternative A: baseline pilot sizes with early negative-evidence stop."""
    policy = "stop"
