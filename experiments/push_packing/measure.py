"""Paired packing evaluation against the ACCEPTED v1.3 incumbent.
...
from experiments.push_overlay.scenarios import CUSTOM_SCENARIOS as OLD_CUSTOM_SCENARIOS, NEW_DEVELOPMENT
from experiments.push_packing.scenarios import NEW_HOLDOUT, fixture as new_fixture
CUSTOM_SCENARIOS = OLD_CUSTOM_SCENARIOS + NEW_HOLDOUT
...
# Baseline is the accepted, unchanged v1.3 Agent, including covered Push.
POLICIES = {"baseline": "agent:Agent", "candidate": "experiments.push_packing.agent:PackedPushAgent"}
...
def source_snapshot():
    ...
    names |= {p.relative_to(ROOT).as_posix() for p in (ROOT / "experiments/push_packing").glob("*") if p.suffix in (".py", ".md")}
...
def pair(scenario, seed, candidate_spec=POLICIES["candidate"]):
    details, audiences, channels, reached = {}, {}, {}, {}
    for policy, spec in {**POLICIES, "candidate": candidate_spec}.items():
        try:
    ...
    prefix = base["trace"]["baseline_prefix"]
    suffix = candidate["raw_final"][len(prefix):]
    def effects(policy):
        result = {}
        for campaign, ids in zip(details[policy]["raw_final"], audiences[policy]):
            key = (campaign["target_tariff"], campaign["channel"])
            result.setdefault(key, set()).update(ids)
        return result
    old_effects, new_effects = effects("baseline"), effects("candidate")
    kept = all(ids <= new_effects.get(key, set()) for key, ids in old_effects.items())
    extra_effect_contacts = sum(len(ids - old_effects.get(key, set())) for key, ids in new_effects.items())
    checks = {
        ...
        "original_base_prefix_unchanged": candidate["raw_final"][:len(prefix)] == prefix,
        "every_incumbent_customer_target_channel_preserved": kept,
        "suffix_free_push": all(c["channel"] == "push" and channels["candidate"]["push"]["cost_per_contact"] == 0 for c in suffix),
        "suffix_subset_of_preexisting_coverage": all(ids <= reached["baseline"] for ids in audiences["candidate"][len(prefix):]),
    ...
    row.update(status="ok" if all(checks.values()) else "invariant_failure", assertions=checks,
               delta=candidate["net"] - base["net"],
               additions=len(candidate["raw_final"]) - len(base["raw_final"]),
               extra_effect_contacts=extra_effect_contacts,
...
def summarize(rows):
    ...
    result["paired"] = dict(median=float(np.median(deltas)), q10=float(np.quantile(deltas, 0.1)),
                            ...
                            mean_campaign_count_delta=float(np.mean([r["additions"] for r in valid])),
...
def main():
    ...
    if out.suffix != ".json" or not out.is_relative_to(ROOT / "reports/push_packing"):
        parser.error("Output must be a new .json inside reports/push_packing/")
